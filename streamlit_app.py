...
def load_model():
    ...
    # CORRECTED UNPACKING: model, train_transform (ignored), val_transform (kept)
...
model, preprocess, tokenizer, device = None, None, None, None
...
try:
    ...
    st.warning("Please refresh the page or check your internet connection.")

# --- Interface ---
# Only show the interface if the model successfully loaded
if model is not None and preprocess is not None:
    ...
    with col1:
        ...
        if uploaded_file:
            ...
            st.image(image, caption="Clinical Image", width=400) # Changed from use_column_width to width to fix deprecation warning
    ...
    st.info("Waiting for model to load before showing interface...")